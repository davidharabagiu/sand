...
std::vector<SHA3Hasher::Byte> SHA3HasherImpl::hash(SHA3_224_t, SHA3Hasher::InputStream &is)
{
    return hash(EVP_sha3_224(), SHA224_DIGEST_LENGTH, IStreamBufIt {is}, IStreamBufIt {});
}

std::vector<SHA3Hasher::Byte> SHA3HasherImpl::hash(SHA3_256_t, SHA3Hasher::InputStream &is)
{
    return hash(EVP_sha3_256(), SHA256_DIGEST_LENGTH, IStreamBufIt {is}, IStreamBufIt {});
}

std::vector<SHA3Hasher::Byte> SHA3HasherImpl::hash(SHA3_384_t, SHA3Hasher::InputStream &is)
{
    return hash(EVP_sha3_384(), SHA384_DIGEST_LENGTH, IStreamBufIt {is}, IStreamBufIt {});
}

std::vector<SHA3Hasher::Byte> SHA3HasherImpl::hash(SHA3_512_t, SHA3Hasher::InputStream &is)
{
