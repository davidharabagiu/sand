    ...
    running_jobs_.insert(io_executer_->add_job(
        [this, promise, count = size_t(count),
            ...
            running_jobs_.insert(executer_->add_job(
                [this, promise, count](const utils::CompletionToken &completion_token) {
                    DEFER({
                        std::lock_guard lock {mutex_};
                        running_jobs_.erase(completion_token);
                    });
                    ...
                    running_jobs_.insert(io_executer_->add_job(
                        [this, promise, peers = std::move(peers),
                            ...
                            std::unique_lock lock {mutex_};
                            running_jobs_.insert(executer_->add_job(
                                [this, promise, new_peers_future, peers = std::move(peers)](
                                    const utils::CompletionToken &completion_token) mutable {
    ...
    running_jobs_.insert(executer_->add_job([this, from, msg](
        ...
        running_jobs_.insert(io_executer_->add_job(
            [this, from, msg,
                ...
                std::unique_lock lock {mutex_};
                running_jobs_.insert(executer_->add_job(
                    [this, from, msg](const utils::CompletionToken &completion_token) {
...
std::future<void> PeerManagerFlowImpl::ping_peers()
{
    ...
    lock.lock();
    running_jobs_.insert(
        io_executer_->add_job([this, promise, ping_futures, peers = std::move(peers)](
                                  const utils::CompletionToken &completion_token) mutable {
            DEFER({
                std::lock_guard lock {mutex_};
                running_jobs_.erase(completion_token);
            ...
            std::unique_lock lock {mutex_};
            running_jobs_.insert(
                executer_->add_job([this, promise, ping_futures, peers = std::move(peers)](
                                       const utils::CompletionToken &completion_token) mutable {
                    for (auto &[a, f] : *ping_futures)
                    {
                    ...
                    running_jobs_.insert(
                        io_executer_->add_job([this, ctx, pull_reply, ping_futures](
                            ...
                            if (ctx->new_peers.size() < ctx->count)
                            {
                                ...
                                std::unique_lock lock {mutex_};
                                running_jobs_.insert(executer_->add_job(
                                    [this, ctx](const utils::CompletionToken &completion_token) {
                                        find_new_peers_loop(ctx);
                                        std::lock_guard lock {mutex_};
                                        running_jobs_.erase(completion_token);
                                    }));
